...
Envoy is used for traffic management, proxying, TLS termination, and service-to-service mediation. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Proxy traffic, terminate TLS, and enforce routing or policy at the edge or between services.
- Define the constraints that shape safe use of Envoy.
- Show how Envoy interacts with the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Envoy belongs in the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Envoy implements traffic management, proxying, TLS termination, and service-to-service mediation through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for traffic management, proxying, TLS termination, and service-to-service mediation.
...
- Functional requirements for Envoy.
...
2. Choose Envoy only if it improves the target quality attributes.
...
- Use Envoy when traffic management, proxying, TLS termination, and service-to-service mediation is a first-order requirement.
...
- Provides a disciplined way to implement traffic management, proxying, TLS termination, and service-to-service mediation.
...
- A cloud-managed load balancer when only basic ingress is needed.
...
- Choose Envoy only when it improves the target quality attributes for traffic management, proxying, TLS termination, and service-to-service mediation.
