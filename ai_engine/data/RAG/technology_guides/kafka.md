...
Kafka is used for durable event streaming, decoupled producers and consumers, and replayable logs. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Move events durably between producers and consumers with replayability and ordering per partition.
- Define the constraints that shape safe use of Kafka.
- Show how Kafka interacts with the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Kafka belongs in the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Kafka implements durable event streaming, decoupled producers and consumers, and replayable logs through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for durable event streaming, decoupled producers and consumers, and replayable logs.
...
- Functional requirements for Kafka.
...
2. Choose Kafka only if it improves the target quality attributes.
...
- Use Kafka when durable event streaming, decoupled producers and consumers, and replayable logs is a first-order requirement.
...
- Provides a disciplined way to implement durable event streaming, decoupled producers and consumers, and replayable logs.
...
- RabbitMQ when task routing and per-message acknowledgments are a better fit.
...
- Choose Kafka only when it improves the target quality attributes for durable event streaming, decoupled producers and consumers, and replayable logs.
