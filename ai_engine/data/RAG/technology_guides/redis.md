...
Redis is used for in-memory caching, fast state lookup, rate limiting, and session support. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Provide in-memory speed for cache-aside, sessions, counters, and rate limits.
- Define the constraints that shape safe use of Redis.
- Show how Redis interacts with the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Redis belongs in the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Redis implements in-memory caching, fast state lookup, rate limiting, and session support through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for in-memory caching, fast state lookup, rate limiting, and session support.
...
- Functional requirements for Redis.
...
2. Choose Redis only if it improves the target quality attributes.
...
- Use Redis when in-memory caching, fast state lookup, rate limiting, and session support is a first-order requirement.
...
- Provides a disciplined way to implement in-memory caching, fast state lookup, rate limiting, and session support.
...
- Keep Redis private and avoid exposing it directly to untrusted networks.
...
- Memcached when a simpler ephemeral cache is enough.
...
- Hot-path caching, token buckets, and short-lived coordination state.
- GitHub and Twitter for Redis-style caching
- Shopify and Wikimedia for Elasticsearch-style search

# Architecture Decision Guidelines

- Choose Redis only when it improves the target quality attributes for in-memory caching, fast state lookup, rate limiting, and session support.
