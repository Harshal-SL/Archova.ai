...
Istio is used for service mesh policy, mTLS, traffic shaping, and telemetry for microservices. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Add mesh policy, mTLS, telemetry, and traffic shaping to Kubernetes services.
- Define the constraints that shape safe use of Istio.
- Show how Istio interacts with the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Istio belongs in the runtime, data, messaging, or traffic-handling layer that a service depends on to meet latency, durability, or integration goals. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Istio implements service mesh policy, mTLS, traffic shaping, and telemetry for microservices through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for service mesh policy, mTLS, traffic shaping, and telemetry for microservices.
...
- Functional requirements for Istio.
...
2. Choose Istio only if it improves the target quality attributes.
...
- Use Istio when service mesh policy, mTLS, traffic shaping, and telemetry for microservices is a first-order requirement.
...
- Provides a disciplined way to implement service mesh policy, mTLS, traffic shaping, and telemetry for microservices.
...
- Managed service that provides a simpler path to service mesh policy, mTLS, traffic shaping, and telemetry for microservices.
...
- Choose Istio only when it improves the target quality attributes for service mesh policy, mTLS, traffic shaping, and telemetry for microservices.
