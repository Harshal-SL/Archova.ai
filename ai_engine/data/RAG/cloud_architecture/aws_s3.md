...
AWS S3 is used for durable object storage for artifacts, media, backups, and data exchange. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Store durable objects for application data, artifacts, backups, and media delivery.
- Define the constraints that shape safe use of AWS S3.
- Show how AWS S3 interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
...
At runtime, AWS S3 implements durable object storage for artifacts, media, backups, and data exchange through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for durable object storage for artifacts, media, backups, and data exchange.
...
- Functional requirements for AWS S3.
...
- A deployable design for AWS S3.
...
2. Choose AWS S3 only if it improves the target quality attributes.
...
- Use AWS S3 when durable object storage for artifacts, media, backups, and data exchange is a first-order requirement.
...
- Provides a disciplined way to implement durable object storage for artifacts, media, backups, and data exchange.
...
- Block storage or a database when atomic updates and queryability are more important than object semantics.
...
- Static asset hosting, backup archives, and event-driven file processing.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS S3 only when it improves the target quality attributes for durable object storage for artifacts, media, backups, and data exchange.
