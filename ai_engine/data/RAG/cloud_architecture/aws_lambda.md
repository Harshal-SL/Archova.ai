...
AWS Lambda is used for event-driven serverless compute with burst scaling and minimal server management. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Execute event-driven code without managing servers or clusters.
- Define the constraints that shape safe use of AWS Lambda.
- Show how AWS Lambda interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

AWS Lambda belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, AWS Lambda implements event-driven serverless compute with burst scaling and minimal server management through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for event-driven serverless compute with burst scaling and minimal server management.
...
- Functional requirements for AWS Lambda.
...
- A deployable design for AWS Lambda.
...
2. Choose AWS Lambda only if it improves the target quality attributes.
...
- Use AWS Lambda when event-driven serverless compute with burst scaling and minimal server management is a first-order requirement.
...
- Provides a disciplined way to implement event-driven serverless compute with burst scaling and minimal server management.
...
- ECS or EKS when long-running processes or custom runtime control is required.
...
- Webhook handlers, event processors, and lightweight API backends.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS Lambda only when it improves the target quality attributes for event-driven serverless compute with burst scaling and minimal server management.
