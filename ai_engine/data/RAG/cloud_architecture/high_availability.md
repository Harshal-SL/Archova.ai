...
High Availability is used for resiliency through redundancy, failover, and health-driven traffic management. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Explain how High Availability contributes to resiliency through redundancy, failover, and health-driven traffic management.
- Define the constraints that shape safe use of High Availability.
- Show how High Availability interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

High Availability belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, High Availability implements resiliency through redundancy, failover, and health-driven traffic management through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for resiliency through redundancy, failover, and health-driven traffic management.
...
- Functional requirements for High Availability.
...
- A deployable design for High Availability.
...
2. Choose High Availability only if it improves the target quality attributes.
...
- Use High Availability when resiliency through redundancy, failover, and health-driven traffic management is a first-order requirement.
...
- Provides a disciplined way to implement resiliency through redundancy, failover, and health-driven traffic management.
...
- Managed service that provides a simpler path to resiliency through redundancy, failover, and health-driven traffic management.
...
- Choose High Availability only when it improves the target quality attributes for resiliency through redundancy, failover, and health-driven traffic management.
