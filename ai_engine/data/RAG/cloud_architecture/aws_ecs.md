...
AWS ECS is used for AWS-native container scheduling for services that need lower orchestration overhead. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Provide container orchestration with lower operational overhead than self-managed Kubernetes.
- Define the constraints that shape safe use of AWS ECS.
- Show how AWS ECS interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

AWS ECS belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, AWS ECS implements AWS-native container scheduling for services that need lower orchestration overhead through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for AWS-native container scheduling for services that need lower orchestration overhead.
...
- Functional requirements for AWS ECS.
...
- A deployable design for AWS ECS.
...
2. Choose AWS ECS only if it improves the target quality attributes.
...
- Use AWS ECS when AWS-native container scheduling for services that need lower orchestration overhead is a first-order requirement.
...
- Provides a disciplined way to implement AWS-native container scheduling for services that need lower orchestration overhead.
...
- EKS when Kubernetes APIs and ecosystem compatibility are mandatory.
...
- Teams that want managed task scheduling and simple service deployment.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS ECS only when it improves the target quality attributes for AWS-native container scheduling for services that need lower orchestration overhead.
