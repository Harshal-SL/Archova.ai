...
AWS EKS is used for managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Run containerized workloads on managed Kubernetes with a clear separation between platform and application ownership.
- Define the constraints that shape safe use of AWS EKS.
- Show how AWS EKS interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

AWS EKS belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, AWS EKS implements managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation.
...
- Functional requirements for AWS EKS.
...
- A deployable design for AWS EKS.
...
2. Choose AWS EKS only if it improves the target quality attributes.
...
- Use AWS EKS when managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation is a first-order requirement.
...
- Provides a disciplined way to implement managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation.
...
- Use multi-AZ node groups and validate node drain behavior during maintenance.
...
- ECS or a fully managed serverless container platform when Kubernetes control is not required.
...
- Platform teams running microservices with standardized deployment and policy controls.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS EKS only when it improves the target quality attributes for managed Kubernetes execution with pod scheduling, cluster scaling, and workload isolation.
