...
AWS RDS is used for managed relational persistence with automated backups, patching, and failover. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Provide managed relational storage with automated patching, backups, and failover.
- Define the constraints that shape safe use of AWS RDS.
- Show how AWS RDS interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

AWS RDS belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, AWS RDS implements managed relational persistence with automated backups, patching, and failover through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for managed relational persistence with automated backups, patching, and failover.
...
- Functional requirements for AWS RDS.
...
- A deployable design for AWS RDS.
...
2. Choose AWS RDS only if it improves the target quality attributes.
...
- Use AWS RDS when managed relational persistence with automated backups, patching, and failover is a first-order requirement.
...
- Provides a disciplined way to implement managed relational persistence with automated backups, patching, and failover.
...
- Aurora or self-managed databases if the workload needs a different operational model.
...
- Transactional systems that need SQL, constraints, and operational simplicity.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS RDS only when it improves the target quality attributes for managed relational persistence with automated backups, patching, and failover.
