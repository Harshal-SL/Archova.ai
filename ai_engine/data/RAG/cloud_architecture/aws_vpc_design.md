...
AWS VPC Design is used for network isolation, routing, and controlled ingress/egress for AWS workloads. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Define the network isolation boundary, routing model, and trust zones for the AWS estate.
- Define the constraints that shape safe use of AWS VPC Design.
- Show how AWS VPC Design interacts with the infrastructure and platform layer, directly under the application and above the cloud provider control plane.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

AWS VPC Design belongs in the infrastructure and platform layer, directly under the application and above the cloud provider control plane. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, AWS VPC Design implements network isolation, routing, and controlled ingress/egress for AWS workloads through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for network isolation, routing, and controlled ingress/egress for AWS workloads.
...
- Functional requirements for AWS VPC Design.
...
- A deployable design for AWS VPC Design.
...
2. Choose AWS VPC Design only if it improves the target quality attributes.
...
- Use AWS VPC Design when network isolation, routing, and controlled ingress/egress for AWS workloads is a first-order requirement.
...
- Provides a disciplined way to implement network isolation, routing, and controlled ingress/egress for AWS workloads.
...
- Separate public, private, and isolated subnets to reduce blast radius.
...
- A single subnet model for very small or non-sensitive workloads.
...
- Multi-tier SaaS platforms that need private app and data tiers.
- Amazon style shared-service account boundaries
- financial workloads with strict network segmentation

# Architecture Decision Guidelines

- Choose AWS VPC Design only when it improves the target quality attributes for network isolation, routing, and controlled ingress/egress for AWS workloads.
