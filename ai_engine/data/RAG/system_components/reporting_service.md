...
Reporting Service is used for scheduled or on-demand reporting over governed business data. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Produce scheduled and ad-hoc reports over governed operational data.
- Define the constraints that shape safe use of Reporting Service.
- Show how Reporting Service interacts with the domain-service layer where bounded responsibilities are exposed through APIs, events, or async jobs.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Reporting Service belongs in the domain-service layer where bounded responsibilities are exposed through APIs, events, or async jobs. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Reporting Service implements scheduled or on-demand reporting over governed business data through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for scheduled or on-demand reporting over governed business data.
...
- Functional requirements for Reporting Service.
...
- A deployable design for Reporting Service.
...
2. Choose Reporting Service only if it improves the target quality attributes.
...
- Use Reporting Service when scheduled or on-demand reporting over governed business data is a first-order requirement.
...
- Provides a disciplined way to implement scheduled or on-demand reporting over governed business data.
...
- Managed service that provides a simpler path to scheduled or on-demand reporting over governed business data.
...
- Choose Reporting Service only when it improves the target quality attributes for scheduled or on-demand reporting over governed business data.
