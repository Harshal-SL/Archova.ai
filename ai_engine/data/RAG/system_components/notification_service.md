...
Notification Service is used for channel orchestration for push, email, SMS, and in-app delivery. In production systems, it should be selected for its operational envelope as much as for its feature set. The real architectural question is whether it reduces risk, improves scalability, or simplifies ownership compared with adjacent choices.

# Purpose

- Coordinate multi-channel notifications with retries, templates, and preference handling.
- Define the constraints that shape safe use of Notification Service.
- Show how Notification Service interacts with the domain-service layer where bounded responsibilities are exposed through APIs, events, or async jobs.
- Provide decision criteria for HLD and LLD reviews.

# Architecture Placement

Notification Service belongs in the domain-service layer where bounded responsibilities are exposed through APIs, events, or async jobs. It usually sits between application logic and the underlying runtime, storage, or cloud primitives, and it should be designed as part of the end-to-end service boundary rather than as a stand-alone utility.

# Internal Working

At runtime, Notification Service implements channel orchestration for push, email, SMS, and in-app delivery through a small set of deterministic controls: request handling, state access, failure behavior, and observability. The key design task is to make those controls explicit so that scale, security, and recovery characteristics are predictable.
...
- Own the narrow contract for channel orchestration for push, email, SMS, and in-app delivery.
...
- A deployable design for Notification Service.
...
2. Choose Notification Service only if it improves the target quality attributes.
...
- Use Notification Service when channel orchestration for push, email, SMS, and in-app delivery is a first-order requirement.
...
- Provides a disciplined way to implement channel orchestration for push, email, SMS, and in-app delivery.
...
- Managed service that provides a simpler path to channel orchestration for push, email, SMS, and in-app delivery.
...
- Choose Notification Service only when it improves the target quality attributes for channel orchestration for push, email, SMS, and in-app delivery.
