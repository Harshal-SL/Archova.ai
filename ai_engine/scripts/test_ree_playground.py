...
def main_menu():
    ...
    while True:
        ...
        if gen_arch in ("y", "yes"):
            print("\nInvoking Architecture Generation Pipeline (run_design_pipeline_from_arsrs)...")
            try:
                from app.services.design_service import run_design_pipeline_from_arsrs
                design_output = run_design_pipeline_from_arsrs(arsrs)
                print_architecture_output(design_output)
            except Exception as exc:
