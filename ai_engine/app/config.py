...
def print_startup_env_diagnostics() -> None:
    """Print loaded model environment variables and API key sanity check on startup."""
    _load_env_file(override=True)
    api_key = os.getenv("OPENROUTER_API_KEY", "")
    key_exists = bool(api_key.strip())
    masked_key = f"{api_key[:12]}..." if len(api_key) > 12 else ("Set (short key)" if key_exists else "NOT SET")

    model_vars = [
        "LLM_MODEL",
        "INPUT_UNDERSTANDING_MODEL",
        "REQUIREMENT_ENGINEER_MODEL",
        "BUSINESS_ANALYST_MODEL",
        "DOMAIN_EXPERT_MODEL",
        "REQUIREMENT_REVIEW_MODEL",
        "INTERVIEW_MODERATOR_MODEL",
        "DESIGN_MODEL",
        "FALLBACK_MODEL",
    ...
    print("=" * 60)
    print(" [REE Startup Configuration Audit]")
    print(f"  .env Loaded        : True ({_PROJECT_ROOT / '.env'})")
    print(f"  OPENROUTER_API_KEY : Exists={key_exists} | Prefix={masked_key}")
    print("  Model Environment Variables:")
    for var_name in model_vars:
        val = os.getenv(var_name, "<UNSET>")
        print(f"    - {var_name:<28}: {val}")
    print("=" * 60)
